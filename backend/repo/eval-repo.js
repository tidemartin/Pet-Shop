...
module.exports = {
    ...
    buildDelete:function(entity){
        let sql=`delete from eval `
        let q=this.buildWhere(entity)
        q.sql=sql+q.sql
        return q
    },
    buildCont:function(entity){
        let sql = `select count(*) from eval `
        let q=this.buildWhere(entity)
        q.sql=sql+q.sql
        return q
    },
    buildSelect:function(entity){
        let sql = `select c.*,u.username as username from Eval c
        ...
        _q.count(contsql,r=>{
            ...
            _q.findList(listsql,r=>{
                ...
                for (let i = 0; i < data.content.length; i++) {
                    ...
                    _q.findList(`select c.*,u.username as username from Eval c left join user u on u.id = c.userId where c.parentId = ${eval.id}`,gcb=>{
    ...
    queryById:function (entity,callback,conn) {
        let sqlStr="select * from eval where id = ?"
        let sql={
            sql:sqlStr,
            params:[entity.id]
        }
    ...
    deleteById:function (entity,callback,conn) {
        let sqlStr="delete from eval where id = ?"
        let sql={
            sql:sqlStr,
            params:[entity.id]
        }
